...
Docker Compose. Redis хранит необязательный кэш публичного меню.
...
- Redis cache-aside для публичного меню с версионными ключами и TTL.
...
flowchart LR
    ...
    platform --> cache[(Redis 7.4<br/>Публичное меню · TTL)]
...
`GET /api/v1/venues/{venueId}/menu` сначала читает `menuVersion` заведения из
PostgreSQL, затем ищет снимок в Redis по ключу
`mealroute:public-menu:v1:{venueId}:{menuVersion}`. При промахе читает меню из
БД, фильтрует недоступные товары и сохраняет публичный снимок на `MENU_CACHE_TTL`
(по умолчанию 60 секунд). Другие endpoints пока не кэшируются.

Обновление меню атомарно увеличивает версию в БД, поэтому новые запросы больше
не используют старый ключ. Старые ключи удаляются по TTL. Даже запоздавшая
запись старого снимка не перезапишет новую версию. Это также не требует
успешного `DEL` при каждом partner menu sync. Запрос, пересекающийся с
обновлением, может вернуть предыдущий снимок; создание заказа всегда проверяет
актуальную версию, цены и наличие непосредственно в PostgreSQL.

Ошибка или недоступность Redis приводит к чтению из БД; Platform не требует
доступности Redis при старте. Таймаут каждой операции задаётся `REDIS_TIMEOUT`
(100 мс по умолчанию), автоматические повторы отключены. После ошибки чтения
запись в кэш в этом запросе пропускается. Невалидный JSON считается промахом.

Compose поднимает Redis без публикации порта на хост, без персистентного volume,
с лимитом памяти 64 МБ и вытеснением `allkeys-lru`. Вне Compose пустой `REDIS_URL`
отключает кэш; при его указании нужны положительные `MENU_CACHE_TTL` и
`REDIS_TIMEOUT`. Compose задаёт внутренний адрес `redis://redis:6379/0`.

Компромисс: небольшой запрос версии в PostgreSQL сохраняется даже при попадании
в Redis. Кэш экономит чтение и декодирование большого JSON меню, но ускорение
на маленьком demo-меню не гарантировано. Метрики попаданий, circuit breaker и
подавление одновременных промахов пока не реализованы.
...
- Расширение Redis-кэша на каталог после нагрузочных измерений.
